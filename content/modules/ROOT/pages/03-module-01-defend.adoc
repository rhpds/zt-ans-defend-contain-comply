...
image::sec1.png[Splunk Search & Reporting]
...
. Navigate to **Settings → Searches, reports, and alerts**. Make sure you configure your filters to **Type: ALL**, **App: All** and **Owner: Administrator**. You will then see the pre-configured **DCC CVE Alert** saved search. This search runs every minute and is configured to fire the EDA add-on action when it detects matches.
...
. In AAP, you need to login with the following details:
...
image::sec5.png[Vulnerability Containment Workflow]
...
. In *Gitea* , click on **Explore** and navigate to the repository `defend-contain-comply`.

. Create a new file: `splunk-cve-alert.yml` in the following path to the repository `extensions/eda/rulebooks`, add the following yaml content:

image::sec6.png[Gitea Repository]
...
image::sec7.png[Gitea Repository]
...
4. Commit your changes in Gitea

5. Now sync the EDA project in the AAP Controller (*Automation Decisions → Projects → Defend, Contain, Comply → Sync*).
...
. In AAP, open the **Automation Decisions** and navigate to **Infrastructure → Credentials**. We need to create a token credential to be used by the Event Stream. Click on **Create credential**:
+
...
. Copy the Event Stream URL and the Token from the Event Stream you created, and save if for later.
...
* Authentication Token: ansibleiscool
* SSL Hostname Verification: unchecked (lab environment uses self-signed certificates)
* Click **Add**


. Navigate back to **AAP**, navigate to **Automation Decisions → Rulebook Activations** and click **Create rulebook activation**:
+
...
* Event stream: Select the Gear Icon, Event stream: Splunk EDA
* Click **save**

image::sec8.png[Event Stream Select]
...
. In AAP, navigate to **Automation Execution → Templates** and find the **DEFEND - Deploy Application** job template.
...
. In AAP, navigate to **Automation Execution → Templates** and launch the **DEFEND - Scan System** job template. When prompted for a **Limit**, enter `rhel02`.
+
...
. Switch to the **AAP Controller** tab and navigate to **Automation Decisions → Event Streams**  — you should see the event received counter go from 0 to 1. 
...
.Navigate to **Automation Decisions → Event Streams → Rule Audit** and verify you have a successful **CVE alert from Splunk** event. Select this event, and then click on **Actions** in the details of the event to see which job was launched.
...
. From the **Terminal** tab Verify containment on rhel02 from the terminal — `eth0` should be isolated while httpd stays running:
...
+
Lastly, navigate back to **Automation Decisions → Event Streams** and disable event forwarding found in the top right.

image::sec9.png[Disable Forwarding]
