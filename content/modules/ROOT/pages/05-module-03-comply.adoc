...
Before running the audit, examine the policies that define Parasol Finance's security baselines. Log back into the Automation Platform as admin.
...
. Identify the 17 controls and their categories:
+
* **SELinux** — Is enforcement mode active?
* **Firewall** — Is firewalld running?
* **SSH** — Root login, X11 forwarding, max auth tries, idle timeout, login banner
* **Kernel** — ICMP redirects, send redirects, ASLR, source routing
* **File permissions** — `/etc/passwd`, `/etc/shadow`, `sshd_config`
* **Services** — Unnecessary services (`rpcbind`) should be disabled
* **Login** — Restrictive `UMASK 027`
...
* You can identify the 17 controls and their categories in the Rego source
...
. In AAP Controller, navigate to **Automation Execution → Templates** and find **COMPLY - Compliance Audit**.
+
...
+
Some controls already pass from earlier modules and the OS default (SELinux, firewall, ASLR, and no open CVEs if you patched in Module 2). The rest fail on purpose: app servers are seeded with a weak CIS posture so the pre-hardening report should show roughly **4 of 17** passing. That gap is what CIS hardening is about to close.
...
. In AAP Controller, navigate to **Automation Execution → Templates** and launch **COMPLY - CIS Harden**.
...
[source,role="execute"]
----
ssh rhel01 "sudo sshd -T 2>/dev/null | grep -E '^(permitrootlogin|x11forwarding|maxauthtries|clientaliveinterval|banner)'"
----
+
Expected:
+
...
* `banner /etc/issue.net` — presents a legal notice before login

. **Verify file permission hardening** — sensitive system files must have restricted permissions:
+
[source,role="execute"]
----
ssh rhel01 'stat -c "%a %n" /etc/passwd /etc/shadow /etc/group /etc/gshadow /etc/ssh/sshd_config'
----
+
Expected: `644` for passwd/group (readable by all, writable by root), `000` for shadow/gshadow (accessible only by root), `600` for `sshd_config`.

. **Verify kernel parameters** — network stack hardening prevents common attack vectors:
+
[source,role="execute"]
----
ssh rhel01 sysctl net.ipv4.conf.all.accept_redirects net.ipv4.conf.all.send_redirects net.ipv4.conf.all.accept_source_route kernel.randomize_va_space
----
+
Expected:
+
...
* `accept_source_route = 0` — drops source-routed packets
...
. **Verify login umask** — new files should not be world-writable:
+
[source,role="execute"]
----
ssh rhel01 "grep ^UMASK /etc/login.defs"
----
+
Expected: `UMASK 027`.
...
+
* The **pre-hardening** report shows roughly **4 of 17** controls passing (SELinux, firewall, ASLR, and no open CVEs)
* The **post-hardening** report should show **17 of 17** passing — CIS hardening closed the SSH, kernel, file, service, and login gaps
+
...
. In AAP Controller, navigate to **Automation Execution → Templates** and launch **COMPLY - Scan Image**.
+
...
. In AAP Controller, navigate to **Automation Execution → Templates** and find the **Compliance and Hardening** workflow template (it has a workflow icon, not a job template icon).
...
. Once complete, click on the report node and  review the `set_stats` artifacts in the details of the job:
+
In AAP Controller, click on the completed workflow job, then click on the **report** node, and select the details and look at the **Extra Vars / Artifacts** panel. You should see data flowing between nodes:
+
