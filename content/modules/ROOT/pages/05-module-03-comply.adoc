...
. In AAP, navigate to **Automation Execution → Templates** and find **COMPLY - Compliance Audit**.
+
...
. In AAP, navigate to **Automation Execution → Templates** and launch **COMPLY - CIS Harden**.
...
. **Verify SSH hardening** — these are the controls most commonly flagged in audits, run the following from the **Terminal** tab:
+
[source,role="execute"]
----
...
* `x11forwarding yes` — disables GUI forwarding (attack surface reduction)
...
+
In AAP, launch **COMPLY - Compliance Audit** again. Set the extra variable:
+
[source,yaml]
----
audit_phase: post
...
+
In AAP, launch **COMPLY - Compliance Report**.
...
. In AAP, navigate to **Automation Execution → Templates** and launch **COMPLY - Scan Image**.
+
...
. In AAP, navigate to **Automation Execution → Templates** and find the **Compliance and Hardening** workflow template (it has a workflow icon, not a job template icon).
...
+
In AAP, click on the completed workflow job, then click on the **report** node, and select the details and look at the **Extra Vars / Artifacts** panel. You should see data flowing between nodes:
+
