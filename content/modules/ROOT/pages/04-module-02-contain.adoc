...
. Go to the *Gitea* tab, navigate to `lab/policies/patch-policy.yml` and read through the Rego policy. The company policy will check for:
+
* Maintenance Window
* Backup Verification
* Disk Space
...
. Commit your changes in Gitea, then sync the project in AAP Controller (*Automation Execution → Projects → DCC Workshop → Sync*). Test by launching the **CONTAIN - Pre-Patch Check** job template and check the output of the Display policy gate results task 

image::sec10.png[Pre-Patch Check Output]

== Exercise 3: Build the Patching Workflow in AAP
...
. In AAP, navigate to **Automation Execution → Templates**.

. Click the **Edit** on the **Policy-Gated Patching** workflow.

. Confirm the workflow template details:
+
...
. After saving, you'll be taken to the workflow details page. Click the **view worklfow visualizer** button.
...
+
* Hover over the **CONTAIN - Pre-Patch Check** node and  click the **three dots/options** icon and select add step and link
...
+
* Hover over the **Approve Patch Application** node and click the **three dots/options** icon and select add step and link
...
+
* Hover over the **CONTAIN - Patch System** node and click the **three dots/options** icon and select add step and link
...
+
* Hover over the **CONTAIN - Post-Patch Verify** node and click the **three dots/options** icon and select add step and link
...
. In AAP Controller, navigate to **Settings → Automation Execution → Policy** and click on the pencil to edit the settings.
...
image::sec11.png[Authorization Error]
...
. We should set the password of the security_engineer, navigate to **Access Management → Users**, and click on the pencil to edit the user. Set the password to:
+
...
. Navigate to **Automation Execution → Templates** and launch the Policy-Gated Patching workflow
...
----
. In AAP, go back to **Automation Execution → Jobs** and click on the completed **Policy-Gated Patching** workflow job. Click on the **Pre-Patch Check**
+
On the job details look at the **Artifacts** panel. The `set_stats` data from each job shows the complete audit trail:
+
* pre_patch_policy_passed: 2
* policy_check_timestamp: 2026-09-02T19:56:42Z2026-09-02T19:56:43Z
* policy_gates_passed: '44'
+
