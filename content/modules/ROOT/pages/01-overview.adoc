...
* SIEM - Security Information and Event Management is a tool that collects and analyzes log data from across an organization's digital environment to spot threats in real time.
* AIDE - Advanced Intrusion Detection Environment is a host-based intrusion detection system used in Red Hat Enterprise Linux (RHEL) to monitor file integrity and detect unauthorized system modifications
* SSRF - Server-Side Request Forgery is a vulnerability that allows an attacker to make a server-side request to a different domain than the one specified in the request.
...
| `aap-controller`
| AAP Controller 2.6 — workflow orchestration
| Web UI tab
